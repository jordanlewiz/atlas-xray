...
async function initializeModels() {
  try {
    ...
    console.log('[AnalysisService] All models loaded successfully');
  } catch (error) {
    console.error('[AnalysisService] Failed to load models:', error);
    throw error;
  }
}
...
export async function analyzeUpdateQuality(
  ...
  if (!pipeline || isContentScript) {
    console.log('🔍 AI analysis not available - NO FALLBACK ALLOWED');
    throw new Error('AI analysis not available in this context - no fallback allowed');
  }
...
async function analyzeUpdateQualityViaBackground(
  ...
  try {
    ...
    if (typeof chrome === 'undefined' || !chrome.runtime) {
      console.warn('⚠️ Chrome API not available - NO FALLBACK ALLOWED');
      throw new Error('Chrome API not available - no fallback allowed');
    }
    ...
    console.log('⚠️ Background script analysis disabled - NO FALLBACK ALLOWED');
    throw new Error('Background script analysis disabled - no fallback allowed');
  } catch (error) {
    console.error('❌ Failed to communicate with background script:', error);
    throw new Error('Background script communication failed - no fallback allowed');
  }
}
...
async function analyzeCriterionWithAI(
  ...
  try {
    ...
    console.warn(`[AnalysisService] Failed to analyze criterion ${criteria.id}:`, error);
...
async function extractAnswerWithAI(
  ...
  try {
    ...
    console.warn('[AnalysisService] AI QA extraction failed:', error);
    return {
...
export const analyzeProjectUpdate = (text: string) => 
  analysisService.analyzeUpdateQuality(text);
